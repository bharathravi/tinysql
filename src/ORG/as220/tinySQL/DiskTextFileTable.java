 ...
 */
public class DiskTextFileTable extends tinySQLTable
{
   ...
   *
   */
  public DiskTextFileTable(String dDir, String table_name, textFileDatabase engine) throws tinySQLException
  {
    super(table_name);
  ...
  private void writeIndex() throws tinySQLException {
    try {
      Utils.delFile(dataDir + "/" + getName() + indext);
      BufferedOutputStream fdef =
          new BufferedOutputStream(
              new FileOutputStream(dataDir + File.separator + getName() + indext
              ));

      // open it as a DataOutputStream
      //
      DataOutputStream def = new DataOutputStream(fdef);
      def.write(getIndexString());
...
//      //Log.debug ("TextFileTable " + getName() + ": ColCount=" + column_info.size());
  ...
  private void readIndex() throws tinySQLException {
    ...
    try {
      ...
      try {
        fdef = new FileInputStream(dataDir + "/" + getName() + indext);
